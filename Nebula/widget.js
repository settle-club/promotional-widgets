class SettlePopupWidget extends HTMLElement {
  ...
  setupEventListeners() {
    ...
    if (this.attributes.selector) {
      ...
      settleOnboardLink = this.shadowRoot.querySelector(
        ".onboard-with-settle-link"
      );
    ...
    settleOnboardLink &&
      settleOnboardLink.addEventListener("click", () =>
        this.handleOnboardToSettle()
      );
  }
  ...
  handleOnboardToSettle = async () => {
    try {
      const merchantId =
        this.getAttribute("merchant-id") || this?.attributes["merchant-id"];
      if (merchantId) {
...
window.showSettleWidget = function (attributes) {
  ...
  let settleWidget = document.querySelector("#settle-popup-widget");
  if (!settleWidget) {
    settleWidget = document.createElement("div");
    settleWidget.setAttribute("id", "settle-popup-widget");
    document.body.appendChild(settleWidget);
  }

  const settleObj = new SettlePopupWidget({
    selector: "#settle-popup-widget",
    ...attributes,
  });

  setTimeout(() => {
    if (settleObj?.showWidget) {
      settleObj.showWidget();
    }
  }, 200);
};
